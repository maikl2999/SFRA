...
/**
 * @module scripts/swell/api/SwellAPI
 *
 * This is main script for Swell Loyalty. It contains all functions to create gift certificates,
 ...
 * also validates the request parameters. It also validates api key for Swell.
 *
 * @param {Object} params : The mandatory parameters for current request.
 * @param {string} params.apiKey - The swell API Key for request validation
...
function fetchCustomers(params) {
    ...
    var logLocation = 'SwellAPI~fetchCustomers';
    var result = {
    ...
    var isValid = YotpoUtils.validateSwellApiKey(params.apiKey);
    if (!isValid) {
        YotpoLogger.logMessage('The Swell API Key could not be validated therefore cannot proceed further', 'error', logLocation);

        result.status = Constants.STATUS_401;
        result.responseJSON = {
            errorDescription: 'Invalid Swell API key provided'
        };
        return result;
    }
    ...
    try {
        ...
        if (params.singleCustomer) {
            var profile = SwellCustomerModel.searchCustomer(params);
            if (profile) {
                result.responseJSON = SwellCustomerModel.prepareCustomerJSON(profile);
            } else {
            ...
            var profileIterator = SwellCustomerModel.searchCustomers(params);
            result.responseJSON = SwellCustomerModel.prepareCustomersJSON(profileIterator);
        }
    } catch (ex) {
...
function getOrdersCount(params) {
    ...
    var logLocation = 'SwellAPI~getOrdersCount';
    var result = {
        status: Constants.STATUS_400,
        responseJSON: {
            errorDescription: 'The request parameters are missing'
    ...
    try {
        ...
        if (params.orderCountByState) {
            var orderState = SwellOrderModel.getOrderState(params.state);
            if (orderState !== null) {
                result.responseJSON = SwellOrderModel.getOrderCountByState(orderState);
            } else {
            ...
            result.responseJSON = SwellOrderModel.getOrderCountByVolume(Constants.ORDER_VOLUME_DAYS);
        }
    } catch (ex) {
...
function fetchOrders(params) {
    ...
    var logLocation = 'SwellAPI~fetchOrders';
    var result = {
        status: Constants.STATUS_400,
        responseJSON: {
            errorDescription: 'The request parameters are missing'
    ...
    try {
        ...
        if (params.singleOrder) {
            ...
            if (order !== null) {
                result.responseJSON = SwellOrderModel.prepareOrderJSON(order);
            } else {
                result.status = Constants.STATUS_404;
                result.responseJSON = {
            ...
            var orderState = SwellOrderModel.getOrderState(params.state);
            if (orderState !== null) {
                var orderIterator = SwellOrderModel.searchOrders(orderState, params.page, params.pageSize);
                result.responseJSON = SwellOrderModel.prepareOrdersJSON(orderIterator);
            } else {
                result.status = Constants.STATUS_400;
                result.responseJSON = {
                    errorDescription: 'The order state is invalid'
...
function createGiftCertificate(params) {
    ...
    var logLocation = 'SwellAPI~createGiftCertificate';
    var result = {
        status: Constants.STATUS_400,
        responseJSON: {
            errorDescription: 'The request parameters are missing'
